	...
	process {
		try {
			@($ComputerName).foreach({
					...
					$blockArgs = $_, $scheduledTaskName, $ForceReboot.IsPresent
					if ($AsJob.IsPresent) {
						$jobParams = @{
							ScriptBlock          = $installProcess
							Name                 = "$_ - WindowsUpdateInstall"
...
function Get-WindowsUpdateInstallResult {
	...
	(
		...
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$ScheduledTaskName = 'WindowsUpdateInstall'
	)
...
function Wait-WindowsUpdate {
	...
	process {
		try {
			if ($updateJobs = (Get-Job -Name '*WindowsUpdateInstall*').where({ $_.State -eq 'Running'})) {
				$timer = Start-Timer
				while ((Microsoft.PowerShell.Core\Get-Job -Id $updateJobs.Id | Where-Object { $_.State -eq 'Running' }) -and ($timer.Elapsed.TotalSeconds -lt $Timeout)) {
					Write-Verbose -Message "Waiting for all WindowsUpdateinstall background jobs to complete..."
					Start-Sleep -Seconds 3
				}
