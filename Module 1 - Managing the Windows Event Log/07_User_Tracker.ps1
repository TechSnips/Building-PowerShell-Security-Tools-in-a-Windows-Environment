<# 
	Ensure all of the appropriate audit policies are applied to ensure all events are recorded
	Advanced AD policies: Audit Logon, Audit Logoff and Audit Other Logon/Logoff Events
	https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/advanced-security-audit-policy-settings
#>

<# 
	Figure out all of the events that define a "logon" and a "logoff" event

	"Logon": Session start, RDP connection, RDP reconnect, unlocked
	"Logoff": Computer turns off, RDP session disconnect, locked
#>
		...
		$events.foreach({
				if ($_.Id -in $SessionStartIds) {
					...
					if (-not $SessionEndEvent) {
						## This be improved by seeing if this is the latest logon event
